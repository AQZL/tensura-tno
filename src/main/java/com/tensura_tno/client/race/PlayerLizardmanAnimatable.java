...
/**
 * Client-only proxy animatable used to render the Tensura lizardman Geo model
 * at a player position.
 */
public final class PlayerLizardmanAnimatable implements GeoAnimatable {

    private static final RawAnimation ATTACK = RawAnimation.begin()
            .then("animation.lizardman.attack", Animation.LoopType.PLAY_ONCE);

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    private final long instanceId;
    private Player player;
    private AnimationController<PlayerLizardmanAnimatable> actionController;
    private boolean wasSwinging;
    private int lastSwingTime = Integer.MIN_VALUE;

    public PlayerLizardmanAnimatable(Player player) {
        this.player = player;
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        this.actionController = new AnimationController<PlayerLizardmanAnimatable>(
                this, "actionController", 0, state -> PlayState.STOP)
                .setAnimationSpeed(1.45D)
    ...
    private PlayState loopController(AnimationState<PlayerLizardmanAnimatable> state) {
        Player p = this.player;
        ...
        if (shouldSit(p)) {
            ...
            name = "animation.lizardman.crouch_walk";
        } else if (shouldCrouch(p)) {
            name = "animation.lizardman.new";
        } else if (isMoving(p)) {
            name = p.isSprinting() ? "animation.lizardman.run" : "animation.lizardman.walk";
        } else {
            name = "animation.lizardman.idle";
        }

        if ("animation.lizardman.new".equals(name)) {
