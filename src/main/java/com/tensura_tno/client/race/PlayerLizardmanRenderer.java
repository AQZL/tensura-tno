...
/**
 * Renders the lizardman Geo model at a player origin instead of the default
 * object-renderer block offset.
 */
public final class PlayerLizardmanRenderer extends GeoObjectRenderer<PlayerLizardmanAnimatable> {

    private static final float LIZARDMAN_ENTITY_WIDTH = 0.6F;
    private static final float LIZARDMAN_ENTITY_HEIGHT = 1.95F;
    private static final float PLAYER_STANDING_HEIGHT = 1.8F;
    private static final float SITTING_Y_OFFSET = 0.65F;

    public PlayerLizardmanRenderer() {
        super(new PlayerLizardmanGeoModel());
        this.addRenderLayer(new PlayerLizardmanArmorLayer(this));
        this.addRenderLayer(new PlayerLizardmanItemLayer(this));
    }

    @Override
    public long getInstanceId(PlayerLizardmanAnimatable animatable) {
        return animatable.getInstanceId();
    }

    @Override
    public void preRender(PoseStack poseStack, PlayerLizardmanAnimatable animatable, BakedGeoModel model,
        ...
        if (player != null && !isInventoryLikeGui()) {
            if (player.isSleeping()) {
                ...
                if (PlayerLizardmanAnimatable.shouldSit(player)) {
    ...
    private static float getRenderWidth(Player player) {
        return player.isSleeping()
                ? LIZARDMAN_ENTITY_WIDTH * player.getScale()
                : player.getBbWidth();
    }
    ...
    private static float computeFitScale(float bbWidth, float bbHeight) {
        float byWidth = bbWidth / LIZARDMAN_ENTITY_WIDTH;
        float byHeight = bbHeight / LIZARDMAN_ENTITY_HEIGHT;
